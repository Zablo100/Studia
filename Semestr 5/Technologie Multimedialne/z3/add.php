...
<?php
 ...
 $result = mysqli_query($link, "SELECT * FROM users3 WHERE username='$user'"); // wiersza, w którym login=login z formularza
 $rekord = mysqli_fetch_array($result); // wiersza z BD, struktura zmiennej jak w BD 
 if(!$rekord) //Jeśli brak, to nie ma użytkownika o podanym loginie, więc możemy go rejestrować
 {
     if($pass == $pass2){
        $sql = "INSERT INTO users3 (username, password) VALUES ('$user', '$pass')";
        if (mysqli_query($link , $sql)){
            mysqli_close($link); // zamknięcie połączenia z BD
            //$_SESSION ['loggedin'] = true;
            $_SESSION['user'] = $user;
            mkdir("users/$user");
            echo "Konto utworzone,<a href=\"index3.php\">zaloguj się tutaj</a> aby przejść do strony logowania.";
        }
     }else{
